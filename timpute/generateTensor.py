...
def createNoise(tensor: np.ndarray, scale: float=1.0):
    """adds noise in-place"""
    return np.random.normal(0, scale, tensor.shape) + np.copy(tensor)


def createUnknownRank(
    drop_perc=0.0, size=(10, 20, 25), distribution="gamma", scale: float=1.0, par=1
):
