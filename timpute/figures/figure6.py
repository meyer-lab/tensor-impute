...
def figure6():
    ...
    ax[1].bar(
        [0, 0.25, 0.5],
        df.loc[("SARS-COV-19 serology", "entry"), "20%"],
        0.25,
        color=[rgbs(i) for i in [0, 1, 2]],
    )
    ax[1].bar(
        [1, 1.25, 1.5],
        df.loc[("SARS-COV-19 serology", "chord"), "20%"],
        0.25,
        color=[rgbs(i) for i in [0, 1, 2]],
    )
    ...
    for m, method in enumerate(METHODNAMES):
        ...
        y = [
            tmp.loc[("DyeDrop profiling", "chord"), (method, f"{int(d*100)}%")]
            for d in DROPS
        ]
    ...
    ax[2].set_title(
        f"Time per Iteration by Chord Masking Percentage, DyeDrop profiling",
    ...
    for t, impType in enumerate(["entry", "chord"]):
        for m, method in enumerate(METHODNAMES[:2]):
            ...
            y = [
                tmp.loc[("HIV serology", impType), ("C-ALS", f"{int(d*100)}%")]
                / tmp.loc[("HIV serology", impType), (method, f"{int(d*100)}%")]
                for d in DROPS
            ]
            ax[3].plot(
                x, y, color=rgbs(m), ls=linestyles[t], label=f"{method}, {impType}"
            )
    ax[3].set_title(
        f"Time per Iteration Decreases Relative to C-ALS\nby Drop Percentage, HIV serology",
        fontsize=TITLE_FONTSIZE,
    )
