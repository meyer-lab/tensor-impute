...
def createDictionary(filename="nonmissing"):
    ...
    with open(
        "./timpute/figures/revision_cache/dataUsage/" + filename + ".pickle", "wb"
    ) as handle:
...
def testMemory(
    ...
    if (
        os.path.isfile(
            "./timpute/figures/revision_cache/dataUsage/" + filename + ".pickle"
        )
        is False
    ):
    ...
    with open(
        "./timpute/figures/revision_cache/dataUsage/" + filename + ".pickle", "rb"
    ) as handle:
        memory = pickle.load(handle)
    memory[dataname][methodname].append(
...
if __name__ == "__main__":
    ...
    if args.dropType and args.dropPerc and args.seed:
        ...
        testMemory(
            SAVENAMES[i], METHODS[j], METHODNAMES[j], max_comps[i], args.filename
        )
