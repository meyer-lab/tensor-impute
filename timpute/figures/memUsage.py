...
def createDictionary(filename="nonmissing"):
    ...
    with open(
        "./timpute/figures/cache/dataUsage/" + filename + ".pickle", "wb"
    ) as handle:
...
def testMemory(
    ...
    if (
        os.path.isfile("./timpute/figures/cache/dataUsage/" + filename + ".pickle")
        is False
    ):
        createDictionary(filename)

    with open(
        "./timpute/figures/cache/dataUsage/" + filename + ".pickle", "rb"
    ) as handle:
