...
def figure3(datalist=savenames):
    ...
    ax[4].set_yscale('log')
    ax[4].legend(loc='lower right', handles=[Line2D([0], [0], label=m, color=rgbs(i)) for i,m in enumerate(methodname)])
    ...
    ax[5].set_yscale('log')
    ax[5].legend(loc='lower right', handles=[Line2D([0], [0], label=m, color=rgbs(i)) for i,m in enumerate(methodname)])
