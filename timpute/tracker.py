...
class Tracker:
    ...
    def __call__(self, tFac, **kwargs):
        ...
        self.total_error[self.rank][self.rep] = np.append(
            self.fitted_error[self.rank][self.rep], calcR2X(tFac, self.data, True)
        )
        if self.mask is not None:
            self.fitted_error[self.rank][self.rep] = np.append(
                self.fitted_error[self.rank][self.rep],
                calcR2X(tFac, self.data, True, self.mask),
            )
            self.imputed_error[self.rank][self.rep] = np.append(
                self.imputed_error[self.rank][self.rep],
                calcR2X(tFac, self.data, True, np.ones_like(self.data) - self.mask),
            )
        if self.track_runtime:
            if self.start is None:
                self.timer[self.rank][self.rep] = np.append(
                    self.timer[self.rank][self.rep], 0.0
                )
            else:
                self.timer[self.rank][self.rep] = np.append(
                    self.timer[self.rank][self.rep], time.time() - self.start
                )
    ...
    def combine(self):
        ...
        for r in self.total_error:
            for i in range(len(self.total_error[r])):
                ...
                if current > max:
                    for j in range(i):
                        self.total_error[r][j] = np.append(
                            self.total_error[r][j], np.full((current - max), np.nan)
                        )
                        self.fitted_error[r][j] = np.append(
                            self.fitted_error[r][j], np.full((current - max), np.nan)
                        )
                        self.imputed_error[r][j] = np.append(
                            self.imputed_error[r][j], np.full((current - max), np.nan)
                        )
                        if self.track_runtime:
                            self.timer[r][j] = np.append(
                                self.timer[r][j], np.full((current - max), np.nan)
                            )
                    max = current
                else:
                    self.total_error[r][i] = np.append(
                        self.total_error[r][i], np.full((max - current), np.nan)
                    )
                    self.fitted_error[r][i] = np.append(
                        self.fitted_error[r][i], np.full((max - current), np.nan)
                    )
                    self.imputed_error[r][i] = np.append(
                        self.imputed_error[r][i], np.full((max - current), np.nan)
                    )
                    if self.track_runtime:
                        self.timer[r][i] = np.append(
                            self.timer[r][i], np.full((max - current), np.nan)
                        )
        ...
        if self.track_runtime:
            self.time_array = self.timer.copy()
        ...
        for r in self.total_error:
            ...
            if self.track_runtime:
                self.time_array[r] = np.vstack(tuple(self.time_array[r]))
        self.combined = True
    ...
    def time_thresholds(self, threshold=0.25, total=False):
        ...
        thres_time = [
            self.time_array[i, thres_iter[ID]] for ID, i in enumerate(met_rows)
        ]
        return thres_time
